...
echo -e "\n${YELLOW}[5/7] Adding Android polyfills and UI overrides...${NC}"
...
HIDE_SIDEBAR_JS_SRC="$PATCHES_DIR/hide-sidebar-button.js"
HIDE_SIDEBAR_JS_DST="$ASSETS_DIR/hide-sidebar-button.js"
HIDE_MCM_JS_SRC="$PATCHES_DIR/hide-mcm-import.js"
HIDE_MCM_JS_DST="$ASSETS_DIR/hide-mcm-import.js"
HIDE_EXPORT_JS_SRC="$PATCHES_DIR/hide-export-wallet.js"
HIDE_EXPORT_JS_DST="$ASSETS_DIR/hide-export-wallet.js"
...
if [ -f "$ANDROID_UI_CSS_SRC" ]; then
    ...
    echo -e "   ${GREEN}✅ android-ui.css copied (optional width override)${NC}"
else
    echo -e "   ${YELLOW}(optional) android-ui.css not found in patches/${NC}"
fi

if [ -f "$HIDE_SIDEBAR_JS_SRC" ]; then
    cp "$HIDE_SIDEBAR_JS_SRC" "$HIDE_SIDEBAR_JS_DST"
    echo -e "   ${GREEN}✅ hide-sidebar-button.js copied (optional sidebar toggle removal)${NC}"
else
    echo -e "   ${YELLOW}(optional) hide-sidebar-button.js not found in patches/${NC}"
fi

if [ -f "$HIDE_MCM_JS_SRC" ]; then
    cp "$HIDE_MCM_JS_SRC" "$HIDE_MCM_JS_DST"
    echo -e "   ${GREEN}✅ hide-mcm-import.js copied (optional MCM import hide)${NC}"
else
    echo -e "   ${YELLOW}(optional) hide-mcm-import.js not found in patches/${NC}"
fi

if [ -f "$HIDE_EXPORT_JS_SRC" ]; then
    cp "$HIDE_EXPORT_JS_SRC" "$HIDE_EXPORT_JS_DST"
    echo -e "   ${GREEN}✅ hide-export-wallet.js copied (optional export section hide)${NC}"
else
    echo -e "   ${YELLOW}(optional) hide-export-wallet.js not found in patches/${NC}"
fi
...
# Remove existing polyfills/hide script tags to re-inject in correct order
CONTENT=$(echo "$CONTENT" | sed '/<script src="\.\/polyfills\.js"><\/script>/d')
CONTENT=$(echo "$CONTENT" | sed '/<script src="\.\/hide-sidebar-button\.js"><\/script>/d')
CONTENT=$(echo "$CONTENT" | sed '/<script src="\.\/hide-mcm-import\.js"><\/script>/d')
CONTENT=$(echo "$CONTENT" | sed '/<script src="\.\/hide-export-wallet\.js"><\/script>/d')

# Build script tags array based on which patch files exist
SCRIPT_TAGS='    <script src="./polyfills.js"></script>'
if [ -f "$HIDE_SIDEBAR_JS_DST" ]; then
    SCRIPT_TAGS="$SCRIPT_TAGS"$'\n''    <script src="./hide-sidebar-button.js"></script>'
fi
if [ -f "$HIDE_MCM_JS_DST" ]; then
    SCRIPT_TAGS="$SCRIPT_TAGS"$'\n''    <script src="./hide-mcm-import.js"></script>'
fi
if [ -f "$HIDE_EXPORT_JS_DST" ]; then
    SCRIPT_TAGS="$SCRIPT_TAGS"$'\n''    <script src="./hide-export-wallet.js"></script>'
fi
