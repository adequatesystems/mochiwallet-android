...
This repository wraps the Mochimo wallet Chrome extension as an Android application. It does **not** contain the wallet logic itself that lives in the upstream `adequatesystems/mochiwallet` repository.
...
mochiwallet-android/              # This repo (Android wrapper only)
 ...
 patches/
    ...
    mobile-ui-panel-button.js # Removes panel button (not available on mobile)
    mobile-ui-mcm-import.js   # Removes "Import MCM File" (not available on mobile)
    mobile-ui-export.js       # Removes Backup/Export section (not available on mobile)
    mobile-ui-main-screen.js  # Main screen customizations (remove backup import, rename mnemonic)
...
# Check APK contents
C:\Android\build-tools\35.0.0\aapt.exe list android/app/build/outputs/apk/debug/app-debug.apk
...
| pnpm | v8.x | Required for extension dependencies (build scripts use pnpm@8) |
...
- The app currently targets SDK 35 (Android 15) to meet Google Play Store requirements
- Compiled with SDK 35 for full Android 15 API access
- **Edge-to-edge display**: Android 15 enforces edge-to-edge by default. The app handles window insets using `WindowCompat.setDecorFitsSystemWindows(window, false)` and applies padding to the WebView to prevent content from drawing behind system bars
- WebView configured with appropriate security policies for modern Android versions
- Tested and verified on Android 15 devices
...
| `patches/mobile-ui-panel-button.js` | Removes panel toggle button (not available on mobile) | Optional |
| `patches/mobile-ui-mcm-import.js` | Removes "Import MCM File" option (not available on mobile) | Optional |
| `patches/mobile-ui-export.js` | Removes Backup/Export section (not available on mobile) | Optional |
| `patches/mobile-ui-main-screen.js` | Main screen: removes backup import, renames mnemonic option | Optional |
