// Mobile UI patch: removes panel toggle button (not applicable in mobile WebView)
// The browser extension panel feature requires Chrome extension APIs unavailable on mobile
(function () {
  var MARK_ATTR = "data-mobile-ui-panel";

  function remove(el) {
    if (!el || el.hasAttribute(MARK_ATTR)) return;
    el.setAttribute(MARK_ATTR, "true");
    el.style.setProperty("display", "none", "important");
    console.log("[Mobile UI] Removed panel button (not applicable on mobile)");
  }
  ...
  function scanAndHide() {
    ...
    headerButtons.forEach(function (btn) {
      ...
      if (rect && lines.length <= 2) {
        console.log("[Mobile UI] Found panel button (rect+line)");
        remove(btn);
        return;
      }
      ...
      if (paths.length > 0 && paths.length <= 3 && !rect && lines.length === 0) {
        ...
        if (hasDotBefore) {
          console.log("[Mobile UI] Found panel button (path)");
          remove(btn);
          return;
        }
      }
