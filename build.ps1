...
Write-Host "`n[5/7] Adding Android polyfills and UI overrides..." -ForegroundColor Yellow
...
$hideSidebarJsSrc = Join-Path $patchesDir "hide-sidebar-button.js"
$hideSidebarJsDst = Join-Path $assetsDir "hide-sidebar-button.js"
$hideMcmJsSrc = Join-Path $patchesDir "hide-mcm-import.js"
$hideMcmJsDst = Join-Path $assetsDir "hide-mcm-import.js"
$hideExportJsSrc = Join-Path $patchesDir "hide-export-wallet.js"
$hideExportJsDst = Join-Path $assetsDir "hide-export-wallet.js"
...
if (Test-Path $androidUiCssSrc) {
    ...
    Write-Host "   ✅ android-ui.css copied (optional width override)" -ForegroundColor Green
} else {
    Write-Host "   (optional) android-ui.css not found in patches/" -ForegroundColor Yellow
}

if (Test-Path $hideSidebarJsSrc) {
    Copy-Item -Path $hideSidebarJsSrc -Destination $hideSidebarJsDst -Force
    Write-Host "   ✅ hide-sidebar-button.js copied (optional sidebar toggle removal)" -ForegroundColor Green
} else {
    Write-Host "   (optional) hide-sidebar-button.js not found in patches/" -ForegroundColor Yellow
}

if (Test-Path $hideMcmJsSrc) {
    Copy-Item -Path $hideMcmJsSrc -Destination $hideMcmJsDst -Force
    Write-Host "   ✅ hide-mcm-import.js copied (optional MCM import hide)" -ForegroundColor Green
} else {
    Write-Host "   (optional) hide-mcm-import.js not found in patches/" -ForegroundColor Yellow
}

if (Test-Path $hideExportJsSrc) {
    Copy-Item -Path $hideExportJsSrc -Destination $hideExportJsDst -Force
    Write-Host "   ✅ hide-export-wallet.js copied (optional export section hide)" -ForegroundColor Green
} else {
    Write-Host "   (optional) hide-export-wallet.js not found in patches/" -ForegroundColor Yellow
}
...
# Normalize script injection for polyfills and optional hides: ensure order before the module popup bundle
$content = $content -replace ' *<script src="\./(polyfills|hide-sidebar-button|hide-mcm-import|hide-export-wallet)\.js"></script>\r?\n?', ''

$scriptTags = @('    <script src="./polyfills.js"></script>')
if (Test-Path $hideSidebarJsDst) { $scriptTags += '    <script src="./hide-sidebar-button.js"></script>' }
if (Test-Path $hideMcmJsDst) { $scriptTags += '    <script src="./hide-mcm-import.js"></script>' }
if (Test-Path $hideExportJsDst) { $scriptTags += '    <script src="./hide-export-wallet.js"></script>' }

$modulePattern = '<script type="module"[^>]*src="\./assets/popup-[^"\s]+\.js"[^>]*></script>'
if ($content -match $modulePattern) {
    Write-Host "   Injecting polyfills and optional hide scripts before popup bundle..." -ForegroundColor Gray
    $content = [regex]::Replace($content, $modulePattern, { param($m) ($scriptTags -join "`n") + "`n" + $m.Value }, 1)
} else {
    Write-Host "   WARNING: module popup script not found; appending polyfills/hide scripts at end of <head>" -ForegroundColor Yellow
